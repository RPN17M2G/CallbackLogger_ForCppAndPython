...
def test_register_function_callback_info_message_received(logger):
    ...
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, "test message", "file.cpp", 42)
    time.sleep(0.1)
...
def test_register_file_callback_info_message_written(logger):
    ...
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, "file log", "file.cpp", 1)
    time.sleep(0.1)
...
def test_register_function_callback_severity_filter_only_error_received(logger):
    ...
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, "should not appear", "f.cpp", 1)
    logger.log(pycallbacklogger.Severity.Error, pycallbacklogger.Component.S, "should appear", "f.cpp", 2)
    time.sleep(0.1)
...
def test_register_function_callback_component_filter_only_M_received(logger):
    ...
    def callback(entry):
        received_entries.append(entry)
    logger.register_function_callback(callback, {pycallbacklogger.Component.M})

    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, "not for M", "f.cpp", 1)
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.M, "for M", "f.cpp", 2)
    time.sleep(0.1)

    assert len(received_entries) == 1
    assert received_entries[0].component == pycallbacklogger.Component.M
...
def test_register_multiple_callbacks_info_message_both_callbacks_receive(logger):
    ...
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, "msg", "f.cpp", 1)
    time.sleep(0.1)
...
def test_log_async_processing_all_messages_received_in_order(logger):
    ...
    # Act
    for i in range(10):
        logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, f"msg{i}", "f.cpp", i)
    time.sleep(0.2)
...
def test_log_no_callbacks_registered_no_error(logger):
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, "no cb", "file.cpp", 1)
    time.sleep(0.05)
...
def test_unregister_callback_callback_not_called(logger): # TODO: Fix this test - add unregister_callback to CallbackLogger
    ...
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, "should not be received", "f.cpp", 1)
    time.sleep(0.05)
...
def test_log_empty_message_callback_receives_empty_string(logger):
    ...
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, "", "f.cpp", 1)
    time.sleep(0.05)
...
def test_log_long_message_callback_receives_full_message(logger):
    ...
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, long_message, "f.cpp", 1)
    time.sleep(0.05)
...
def test_log_special_characters_callback_receives_message(logger):
    ...
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, special_message, "f.cpp", 1)
    time.sleep(0.05)

    assert received_messages == [special_message]

def test_log_multithreaded_logging_all_messages_received(logger):
    import threading
    received_messages = []
    logger.register_function_callback(lambda entry: received_messages.append(entry.message), pycallbacklogger.Severity.Debug)
    def log_many_messages(thread_id):
        for i in range(50):
            logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, f"t{thread_id}_{i}", "f.cpp", i)
    threads = [threading.Thread(target=log_many_messages, args=(n,)) for n in range(4)]
    ...
    assert len(received_messages) == 200
...
def test_log_all_severities_and_components_all_combinations_received(logger):
    ...
    components = [
        pycallbacklogger.Component.S,
        pycallbacklogger.Component.M,
        pycallbacklogger.Component.P,
    ]
...
def test_log_after_logger_deleted_no_crash(logger): # TODO: Check why the hang happens
    ...
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, "before del", "f.cpp", 1)
...
def test_register_file_callback_invalid_path_no_crash(logger):
    ...
    # Act
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, "should not crash", "f.cpp", 1)
    time.sleep(0.05)
...
def test_register_function_callback_callback_raises_exception_callback_called(logger):
    ...
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, "test", "f.cpp", 1)
    time.sleep(0.05)
...
def test_register_function_callback_severity_filter_edge_cases_only_warning_and_error_received(logger):
    ...
    logger.log(pycallbacklogger.Severity.Debug, pycallbacklogger.Component.S, "debug", "f.cpp", 1)
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, "info", "f.cpp", 2)
    logger.log(pycallbacklogger.Severity.Warning, pycallbacklogger.Component.S, "warn", "f.cpp", 3)
    logger.log(pycallbacklogger.Severity.Error, pycallbacklogger.Component.S, "error", "f.cpp", 4)
    time.sleep(0.05)
...
def test_register_function_callback_component_filter_multiple_only_selected_received(logger):
    ...
    logger.register_function_callback(lambda entry: received_components.append(entry.component), pycallbacklogger.Component.S)
    logger.register_function_callback(lambda entry: received_components.append(entry.component), pycallbacklogger.Component.M)

    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, "S", "f.cpp", 1)
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.M, "M", "f.cpp", 2)
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.P, "P", "f.cpp", 3)
    time.sleep(0.05)

    assert pycallbacklogger.Component.S in received_components
    assert pycallbacklogger.Component.M in received_components
    assert pycallbacklogger.Component.P not in received_components
...
def test_register_function_callback_combined_severity_component_filter_only_error_M_received(logger):
    ...
    logger.log(pycallbacklogger.Severity.Error, pycallbacklogger.Component.M, "should appear", "f.cpp", 1)
    logger.log(pycallbacklogger.Severity.Error, pycallbacklogger.Component.S, "should not appear", "f.cpp", 2)
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.M, "should not appear", "f.cpp", 3)
    time.sleep(0.05)

    assert (pycallbacklogger.Severity.Error, pycallbacklogger.Component.M) in received_pairs
...
def test_register_function_callback_same_callback_multiple_times_unregister_one_only_one_receives(logger):
    ...
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, "msg2", "f.cpp", 2)
    time.sleep(0.05)
...
def test_register_function_callback_severity_as_list_only_selected_received(logger):
    ...
    logger.log(pycallbacklogger.Severity.Info, pycallbacklogger.Component.S, "info", "f.cpp", 1)
    logger.log(pycallbacklogger.Severity.Error, pycallbacklogger.Component.S, "error", "f.cpp", 2)
    logger.log(pycallbacklogger.Severity.Debug, pycallbacklogger.Component.S, "debug", "f.cpp", 3)
    time.sleep(0.05)
...
def test_register_function_callback_component_as_list_only_selected_received(logger):
    ...
    logger.register_function_callback(lambda entry: received_messages.append(entry.message), pycallbacklogger.Component.S)
    logger.register_function_callback(lambda entry: received_messages.append(entry.message), pycallbacklogger.Component.M)
